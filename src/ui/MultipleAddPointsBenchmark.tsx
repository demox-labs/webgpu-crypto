...
interface MultipleAddPointsProps {
  name: string;
  inputsGenerator: (inputSize: number) => any[][];
  gpuFunc: (points: ExtPointType[], fieldMath: FieldMath) => Promise<Uint32Array>;
  gpuInputConverter: (points: bigint[]) => [ExtPointType[], FieldMath];
  gpuResultConverter?: (results: bigint[]) => string[];
  wasmFunc: (points: ExtPointType[], fieldMath: FieldMath) => Promise<Uint32Array>;
  wasmInputConverter: (points: bigint[]) => [ExtPointType[], FieldMath];
  wasmResultConverter?: (results: bigint[]) => string[];
}

export const MultipleAddPointsBenchmark: React.FC<MultipleAddPointsProps> = (
  ...
  const runWasm = async () => {
    const wasmInputs = wasmInputConverter(inputs[0]);
    setWasmRunning(true);
    const wasmStart = performance.now();
    const result = await wasmFunc(wasmInputs[0], wasmInputs[1]);
    ...
    const results = wasmResultConverter ? wasmResultConverter(bigIntResult) : bigIntResult.map(r => r.toString());
    console.log(results);
  ...
  const runGpu = async () => {
    const gpuInputs = gpuInputConverter(inputs[0]);
    setGpuRunning(true);
