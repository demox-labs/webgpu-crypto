...
interface BucketAddPointsBenchmarkProps {
  name: string;
  ...
  wasmFunc: (points: ExtPointType[], scalars: number[], fieldMath: FieldMath) => Promise<Uint32Array>;
  wasmInputConverter: (scalars: bigint[]) => [ExtPointType[], number[], FieldMath];
  wasmResultConverter?: (results: bigint[]) => string[];
}

export const BucketAddPointsBenchmark: React.FC<BucketAddPointsBenchmarkProps> = (
  ...
  const runWasm = async () => {
    const wasmInputs = wasmInputConverter(inputs[1]);
    setWasmRunning(true);
    const wasmStart = performance.now();
    const result = await wasmFunc(wasmInputs[0], wasmInputs[1], wasmInputs[2]);
