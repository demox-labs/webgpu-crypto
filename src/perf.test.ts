...
const realCiphertexts = jsonRecords.result.map(record => record.record_ciphertext);
...
const benchmarkBulkIsOwner = async (
  ...
  numRuns: number
    ...
    const pileOfCiphertexts = realCiphertexts.slice(0, bulkRecords);
...
describe('Benchmarking', () => {
  it('should benchmark bulkIsOwner algorithms', async () => {
    ...
    // const pileOfOwnedCiphertexts = copyCiphertext(ownedCiphertext, bulkRecords);
    // const pileOfNotO
    const wasm_bulkIsOwnerSlow_benchmark_all_owned = await benchmarkBulkIsOwner(aleoSDKDecryptorBulkIsOwnerSlow, viewKey, numRuns);
    ...
    const wasm_bulkIsOwner_benchmark_all_owned = await benchmarkBulkIsOwner(aleoSDKDecryptorBulkIsOwner, viewKey, numRuns);
