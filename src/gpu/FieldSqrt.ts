...
fn field_sqrt(num: Field) -> Field {
  ...
  var r: Field = gen_field_pow(num, r_initial_exponent, ALEO_FIELD_ORDER);
  var t: Field = gen_field_pow(num, q, ALEO_FIELD_ORDER);
  var m: Field = s;
  ...
  while (!equal(t, U256_ONE)) {
    ...
    var b_exp: Field = gen_field_pow(U256_TWO, b_exp_exp, ALEO_FIELD_ORDER_MINUS_ONE);
    var b: Field = gen_field_pow(c, b_exp, ALEO_FIELD_ORDER);
