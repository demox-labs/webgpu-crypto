...
export const poseidonHash = (fieldElements: string[]): string => {
  const fieldElementsBigInt = fieldElements.map((fieldElement) => BigInt(fieldElement));
  const result = poseidon(fieldElementsBigInt);
